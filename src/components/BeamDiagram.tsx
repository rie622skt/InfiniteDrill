...
function FixedEndWall({ xLeft }: { xLeft: number }) {
  ...
  return (
    <>
      <Line x1={xLeft} y1={BEAM_Y} x2={xLeft} y2={SUPPORT_BOTTOM_Y} stroke="#333" strokeWidth={2} />
...
function ConcentratedDiagram({
  ...
  return (
    <>
      ...
      <Text x={(xLoad + xRight) / 2} y={DIM_AB_Y + DIM_LABEL_OFFSET} fill="#333" fontSize={12} textAnchor="middle">
        b = {b} m
      </Text>
      {/* L dimension */}
...
export function BeamDiagram({ problem }: BeamDiagramProps) {
  const L = problem.L;
  const beamWidth = L * PIXELS_PER_METER;
